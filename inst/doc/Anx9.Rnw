...
\title{Température à Nottingham Castle (compléments du Chapitre 9)}
...
Nous avons vu que les modèles SARIMA et ARMAX ont des variances résiduelles et des qualités prédictives intra-échantillon identiques.
Pour comprendre cette proximité :
\begin{itemize}
  \item calculer les différences saisonnières de chaque variable explicative (matrice xmat1a) et examiner quelques lignes de cette matrice de différences saisonnières ;
  \item calculer   la moyenne et la variance de chaque série obtenue ;
  \item calculer la différence saisonnière de \code{nott1}, la  moyenne et la variance de cette série filtrée. Expliquer.
\end{itemize}
\end{Exercice}
...
# élimination de la colonne nulle
...
# séparation des intervalles d'estimation et de prévision
...
Les séries transformées sont nulles.  Elles ont été absorbées par le filtre de différence saisonnière, ou encore, ``les fonctions
$\cos(\omega t) $ et $ \sin(\omega t)$ de période 12 sont vecteurs propres associées à la valeur propre 0 pour ce filtre''.
