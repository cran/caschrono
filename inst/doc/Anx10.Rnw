...
\title{Consommation d'électricité (compléments du Chapitre 10)}
...
\begin{exercice}
Superposer les chronogrammes de   \code{u}, défini section 1 et   \code{u.3c}. Commenter.
\end{exercice}
...
\noindent
Nous   formons le data frame des variables de la période d'apprentissage  :
...
Nous avons besoin du détail de l'estimation conduisant à \code{u.3c} :
...
Enfin nous superposons les deux résidus :
...
<<zfiginclude>>
@
\caption{Chronogrammes superposés des résidus u et u.3c.}\label{conselec.super}
\end{figure}

On observe que les résidus \code{u} et  \code{u.3c} sont  très proches (fig. \ref{conselec.super}) alors qu'il y a une 
variable explicative de la régression MCO \code{t1.2} absente de la régression MCG avec erreur SARMA$(1,0)(1,1)_{12}$.
...
\end{split}
 \end{eqnarray}
  est contenu dans l'objet \code{mdarx3c}. Cet objet contient également la matrice des covariances estimée des estimateurs des paramètres.
  Notons $\Sigma$ la sous-matrice des covariances des estimateurs de  $\beta_{cldd}$ et $\beta_{htdd}$. 
  
 Sous l'hypothèse  : $\beta_{cldd} = 10 \;\beta_{htdd}$, 
...
\]
sous $H_0$ elle suit approximativement une loi $\nor(0,1)$. On  remplace  dans $\sigma^2$, tous les paramètres par leurs estimations.
On rejettera l'hypothèse nulle pour de grandes valeurs absolues de $t$.
Nous sommes maintenant en mesure de calculer $t$.
Par \code{str(mdarx3c)} nous repérons   les estimations des paramètres, élément \verb"mdarx3c$coef",  et leur matrice des covariances estimées,  élément 
...
La p.value est supérieure à 18\% ; on peut conserver cette hypothèse.
