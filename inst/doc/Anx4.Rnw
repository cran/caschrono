...
colnames(a12)= c("Retard","p-val. série compl.",
 "p-val. 600 obs.", "p-val. rdt carr\'e")
@
}
...
du rendement de Danone et de son carr\'e.",label="danoblanc", 
digits=4)
@
...
% ###################################################################
\begin{Exercice}
\code{armasubsets()} de  \pkg{TSA} aide \`a l'identification des mod\`eles ARMA suivant le m\^eme principe que \code{armaselect()} mais reconna\^{\i}t les trous (les plages de coefficients nuls)
dans les ordres de r\'egression.
...
Les zones  les plus sombres du graphique (fig. \ref{id.saiso}) sugg\`erent les ordres \`a retenir mais l'interpr\'etation n'est pas simple.
