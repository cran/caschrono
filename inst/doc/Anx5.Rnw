...
<<label=period>>=
# s\'erie de 48 points
# x1 et x2 sont les fonctions p\'eriodiques
# et on v\'erifie que leurs diff\'erences saisonni\`eres sont nulles.
temps = 1:48
x1 = cos(2*pi*temps/4)
diff(x1,4)
x2 = sin(2*pi*temps/4)
...
# terme autor\'egressif complet jusqu'au retard 14
...
lag.plot(rev(y1),4,layout=c(2,2),do.lines=FALSE,
   main="D\'erive n\'egative",diag.col="red")
c=.2
y0=arima.sim(list(order=c(1,0,0),ar=.9),n=200)+c
y1=diffinv(y0,4)[-(1:4)]
lag.plot(rev(y1),4,layout=c(2,2),ask=NA,do.lines=FALSE,
                     main="D\'erive positive",diag.col="red")
@
}
